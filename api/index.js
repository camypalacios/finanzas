// api/index.js — Router único · 1 sola serverless function
// vercel.json enruta "/api/(.*)" → "/api/index?path=$1"
...
export default async function handler(req, res) {
  ...
  // path puede venir como string "auth/login" (desde rewrite) o array ["auth","login"] (catch-all)
  const rawPath = req.query.path || '';
  const seg = (Array.isArray(rawPath) ? rawPath : rawPath.split('/')).filter(Boolean);
